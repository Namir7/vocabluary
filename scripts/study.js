...
//server request 
function serverRequest(urlDB) {
  let urls = [urlDB + "/cards", urlDB + "/studyDeck"];
  let deckAndStudyQueue = urls.map((url) =>
    fetch(url).then((response) => {
      return response.json();
    })
  );
  Promise.all(deckAndStudyQueue)
    .then(([deck, queue]) => {
      //add first card on page
      changeCard(deck[queue[0]]);
      //next btn for other cards
      let currCardIndex = 1;
      nextBTN.addEventListener("click", () => {
        if (currCardIndex === queue.length) {
          card.classList.add("hidden");
          nextBTN.classList.add("hidden");
          finishWritting.classList.remove("hidden");
        } else {
          changeCard(deck[queue[currCardIndex]]);
          currCardIndex++;
        }
      });
    })
    .catch((e) => {
      if (e instanceof TypeError) {
        console.log(`Sorry, network Error`);
      }
    });
}
...
function changeCard(cardObject) {
  ...
  changeColors(cardObject);

  function hideContentApperShowBTN() {
    showBtn.classList.remove("hidden");
    hint.classList.add("hidden");
  }
  function changeColors(cardObject) {
    //changable elements
